...
var authHelpersRemix = require('@supabase/auth-helpers-remix');
var server = require('@remix-run/express/dist/server');
...
// node-file:/Users/andrepato/projects/home-project-planner/node_modules/fsevents/fsevents.node
var require_fsevents = __commonJS({
  "node-file:/Users/andrepato/projects/home-project-planner/node_modules/fsevents/fsevents.node"(exports, module) {
    init_fsevents();
    try {
...
var { SUPABASE_URL = "", SUPABASE_KEY = "" } = process.env;
...
var supabaseServer = (request, response) => {
  return authHelpersRemix.createServerClient(SUPABASE_URL, SUPABASE_KEY, {
    request,
    response
  });
};
function getLoadContext(request, response) {
  response.append("url", "http://localhost:3000");
  const remixRequest = server.createRemixRequest(request, response);
  return {
    supabase: (resp) => supabaseServer(remixRequest, resp)
  };
}
node.installGlobals();
var BUILD_PATH = path__default.default.resolve("./build/index.js");
var build = __require(BUILD_PATH);
var app = express__default.default();
app.use(compression__default.default());
app.disable("x-powered-by");
app.use("/", express__default.default.static("public/build", { immutable: true, maxAge: "1y" }));
app.use(express__default.default.static("public", { maxAge: "1h" }));
app.all(
  "*",
  process.env.NODE_ENV === "development" ? createDevRequestHandler() : express$1.createRequestHandler({
    build,
...
app.listen(port, () => {
  ...
  if (process.env.NODE_ENV === "development") {
    console.log(`\u{1F4E2} Broadcasting changes to dev server \u{1F4E2}`);
...
function createDevRequestHandler() {
  ...
  chokidar.watch(BUILD_PATH, { ignoreInitial: true }).on("add", handleServerUpdate).on("change", handleServerUpdate).on("unlink", handleServerUpdate);
